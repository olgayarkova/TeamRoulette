...
## Members variable names
  ...
  for (i in 1:n){
    ...
    vars$newvars[55+inc*(i-1)]<- paste(m, 'TOEFL.Test.Experience', sep = '')
    vars$newvars[56+inc*(i-1)]<- paste(m, 'CAE.CPE.Test.Experience', sep = '')
    vars$newvars[57+inc*(i-1)]<- paste(m, 'GRE.Test.Experience', sep = '')
    vars$newvars[58+inc*(i-1)]<- paste(m, 'GMAT.Test.Experience', sep = '')
